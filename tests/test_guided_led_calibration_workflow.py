...
from nanoleaf_sync.runtime.color_processing import LedCalibration, apply_color_style_mapping_with_diagnostics, apply_led_calibration, color_pipeline_diagnostics
...
def test_black_and_dark_grey_behavior_with_led_black_cutoff() -> None:
    black = apply_led_calibration(np.asarray([[0.0, 0.0, 0.0]], dtype=np.float32), LedCalibration(black_luminance_cutoff=0.004, black_luminance_knee=0.002))[0]
    dark = apply_led_calibration(np.asarray([[20.0, 20.0, 20.0]], dtype=np.float32), LedCalibration(black_luminance_cutoff=0.004, black_luminance_knee=0.002))[0]
...
def test_medium_grey_and_white_are_neutral_and_visible() -> None:
    medium = apply_led_calibration(np.asarray([[128.0, 128.0, 128.0]], dtype=np.float32), LedCalibration())[0]
    white = apply_led_calibration(np.asarray([[255.0, 255.0, 255.0]], dtype=np.float32), LedCalibration())[0]
...
def test_reference_chroma_and_low_saturation_remain_controlled() -> None:
    saturated, _ = apply_color_style_mapping_with_diagnostics(np.asarray([(235, 80, 40)], dtype=np.float32), color_style="reference")
    sat_diag = color_pipeline_diagnostics(input_rgb=(235, 80, 40), output_rgb=tuple(int(v) for v in saturated[0].tolist()), color_style="reference")
    muted, _ = apply_color_style_mapping_with_diagnostics(np.asarray([(108, 116, 126)], dtype=np.float32), color_style="reference")
    muted_diag = color_pipeline_diagnostics(input_rgb=(108, 116, 126), output_rgb=tuple(int(v) for v in muted[0].tolist()), color_style="reference")
...
def test_calibration_profiles_save_and_load(tmp_path) -> None:
    ...
    cfg = AppConfig(
        display_preset="sdr",
        led_calibration_profile_sdr=LedCalibrationProfile(red_gain=1.1, blue_gain=0.9, black_luminance_cutoff=0.004),
        led_calibration_profile_hdr=LedCalibrationProfile(red_gain=1.0, blue_gain=1.0, black_luminance_cutoff=0.0032),
    )
