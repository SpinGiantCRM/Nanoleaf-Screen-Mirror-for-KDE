...
def test_effective_zone_count_uses_manual_device_count_then_default() -> None:
    assert (
        effective_zone_count(
            config=AppConfig(zones=[], device_zone_count=9), detected_device_zone_count=None
        )
        == 9
    )
    assert (
        effective_zone_count(
            config=AppConfig(zones=[], device_zone_count=0), detected_device_zone_count=7
        )
        == DEFAULT_DERIVED_ZONE_COUNT
    )
    assert (
        effective_zone_count(
            config=AppConfig(zones=[], device_zone_count=0), detected_device_zone_count=None
        )
        == DEFAULT_DERIVED_ZONE_COUNT
    )
...
def test_derive_source_zones_preserves_explicit_configured_zones() -> None:
    cfg = AppConfig(
        zones=[ZoneConfig(x=0.0, y=0.0, w=1.0, h=1.0)], layout_preset="horizontal_debug"
    )
...
def test_edge_strip_48_zone_layout_covers_all_perimeter_sides() -> None:
    cfg = AppConfig(zones=[], layout_preset="edge_strip", device_zone_count=48)
    zones = derive_source_zones(
        config=cfg, detected_device_zone_count=None, frame_width=1920, frame_height=1080
    )
...
def test_runtime_preview_reports_aspect_weighted_side_counts() -> None:
    cfg = AppConfig(
        zones=[], layout_preset="edge_strip", edge_locality="tight", device_zone_count=48
    )
    artifacts = derive_source_zone_artifacts(
        config=cfg, detected_device_zone_count=48, frame_width=1920, frame_height=1080
    )
