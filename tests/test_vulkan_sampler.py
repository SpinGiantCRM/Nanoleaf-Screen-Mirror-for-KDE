...
def test_vulkan_probe_disabled_by_env(monkeypatch) -> None:
    ...
    status = VulkanZoneSampler.probe()
...
@patch("nanoleaf_sync.capture.vulkan_sampler._vulkan_available", return_value=True)
@patch("nanoleaf_sync.capture.vulkan_sampler._vulkan_import_dma_buf_image", return_value=True)
@patch(
    "nanoleaf_sync.capture.vulkan_sampler._vulkan_dispatch_zone_sampler",
...
def test_vulkan_import_and_dispatch(_dispatch, _import, _available, monkeypatch) -> None:
    ...
    monkeypatch.setenv("NANOLEAF_VULKAN_FORCE_AVAILABLE", "1")
...
def test_vulkan_falls_back_when_init_fails(monkeypatch) -> None:
    monkeypatch.setenv("NANOLEAF_ENABLE_VULKAN_SAMPLER", "1")
    with patch("nanoleaf_sync.capture.vulkan_sampler._vulkan_available", return_value=False):
