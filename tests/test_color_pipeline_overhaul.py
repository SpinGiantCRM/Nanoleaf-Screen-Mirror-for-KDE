...
from nanoleaf_sync.runtime.compositor import apply_zone_sdr_boost
...
def test_sdr_boost_applied_before_style_mapping_in_pipeline() -> None:
    ...
    colors, sampled, _pre, final, _timings = out  # type: ignore[misc]
    assert colors
    expected = apply_zone_sdr_boost(
        sampled[0:1].astype(np.uint8),
        sdr_boost_nits=160.0,
        hdr_max_nits=1000.0,
    )[0]
    assert int(final[0, 0]) <= int(expected[0]) + 8
