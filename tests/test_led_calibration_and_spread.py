...
def test_chroma_compression_reduces_oversaturation() -> None:
    ...
    low_diag = color_pipeline_diagnostics(input_rgb=(240, 90, 40), output_rgb=tuple(int(v) for v in np.rint(low)))
    high_diag = color_pipeline_diagnostics(input_rgb=(240, 90, 40), output_rgb=tuple(int(v) for v in np.rint(high)))
...
def test_reference_natural_chroma_is_bounded() -> None:
    for style in ("reference", "natural"):
        styled, _ = apply_color_style_mapping_with_diagnostics(np.asarray([(235, 80, 40)], dtype=np.float32), color_style=style)
        diag = color_pipeline_diagnostics(input_rgb=(235, 80, 40), output_rgb=tuple(int(v) for v in styled[0].tolist()))
        assert float(diag["chroma_ratio"]) <= 1.05


def test_low_saturation_colour_stays_low_saturation() -> None:
    styled, _ = apply_color_style_mapping_with_diagnostics(np.asarray([(118, 122, 128)], dtype=np.float32), color_style="natural")
    diag = color_pipeline_diagnostics(input_rgb=(118, 122, 128), output_rgb=tuple(int(v) for v in styled[0].tolist()))
