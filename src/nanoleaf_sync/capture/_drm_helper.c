        ...
        for (int fd_num = 0; fd_num < 512; fd_num++) {
            ...
            if (pass == 0) {
                ...
                if (card_path && strcmp(link, card_path) == 0) {
                    snprintf(found_card, sizeof(found_card), "%s", card_path);
                    found_fd_num = fd_num;
                    goto clone_it;
                }
            } else {
                ...
                if (strncmp(link, "/dev/dri/card", 13) == 0 && link[13] >= '0' && link[13] <= '9') {
                    snprintf(found_card, sizeof(found_card), "%s", link);
                    found_fd_num = fd_num;
                    goto clone_it;
                }
            }
...
clone_it:
    ...
    if (!card_path || strcmp(found_card, card_path) != 0) {
    ...
    if (out_card_path && found_card[0] != '\0') {
